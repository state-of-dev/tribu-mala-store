...
// Order Status Types (Simplificado - 4 estados)
...
export const getOrderStatusColor = (status: string): string => {
  switch (status as OrderStatus) {
    ...
    case 'CANCELLED': return 'bg-red-500/10 text-red-500 border-red-500/20'       // Rojo para cancelado
...
export const getOrderStatusIcon = (status: string): LucideIcon => {
  switch (status as OrderStatus) {
    ...
    case 'CANCELLED': return XCircle     // X para cancelado
